...
def drawDiagnostic(name, ifile1, mass):
	gROOT.SetBatch(True)
	Sigs = [["/home/akobert/CMSSW_11_1_0_pre7/src/RData/2017/NanoTool_UL/M150_UL_nano_merged.root"], "4.148*7639.0/567896.0", "", "sig150_nano", "150 GeV Signal", "7639.0"]
	sig_name = "sig150_nano"
	ptBins = [220, 230, 255, 290, 360, 1000]
...
#			for P in ["prefit", "fit_b"]:
					...
					cNRES = CbnF.Get("shapes_"+P+"/"+NAME+"/NonRes")	#Non-Resonant Background
				...
				#	cTT = cTT.Clone()
					...
					if run_code != 2:
						...
						cSIG = CbnF.Get("shapes_prefit/"+NAME+"/total_signal")	# Take pre-fit signal to scale
...
#					C2e.Print("results/"+NAME+"/Sig2D_"+P+".png")
					...
					if P == "fit_b":
						savename2 = "sig_sdm_"+str(ptBins[a])+"-"+str(ptBins[a+1])
...
#					C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_v5.root")
					...
					if pf == "pass" and P == "fit_b":
						b_pull_count.SetTitle("fit_b Passing Pull Count "+str(ptBins[a])+"-"+str(ptBins[a+1])+" GeV")
					...
					if P == "fit_b":
						...
						l2.Draw("same")
						p22.RedrawAxis()
						C.Print("results/"+NAME+"/"+savename2+"_"+pf+str(a)+"_"+str(name)+".png")
						p12.cd()
						gPad.SetLogy()
						p12.Draw()
						C.Print("results/"+NAME+"/"+savename2+"_"+pf+str(a)+"_"+str(name)+"_logy.png")
						if pf == "pass":
							s_pull_count.SetTitle("sig Passing Pull Count "+str(ptBins[a])+"-"+str(ptBins[a+1])+" GeV")
