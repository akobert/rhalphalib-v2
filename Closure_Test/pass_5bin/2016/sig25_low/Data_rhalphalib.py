...
def plot_mctf(tf_MCtempl, msdbins, GJeff):
    """
    ...
    tf_MCtempl_vals = tf_MCtempl(ptpts_scaled, rhopts_scaled, nominal=True) * GJeff
    df = pd.DataFrame([])
    ...
    ROOT.gInterpreter.Declare("Double_t widebins10[6] = {200, 230, 255, 285, 335, 700};")
    mpt_TF = TH2F("mpt_TF", "Mass vs. pT Transfer Factor", 5, widebins10, 40, 0, 2000)
...
#	print(mpt[i][2])
    ...
    cb.set_label("Ratio")
    fig.savefig("MCTF_msdpt_2016_sig25v5.png",bbox="tight")
    plt.clf()
...
def test_rhalphabet(tmpdir,
    ...
    # define bins    
    ptbins = np.array([200, 230, 255, 285, 335, 700])
    npt = len(ptbins) - 1
    ...
    msdbins = np.linspace(0, 200, 41) 
    plot_mctf(tf_MCtempl,msdbins,GJeff)
    ...
    tf_dataResidual = rl.BasisPoly('CMS_EXO24027_tf_dataResidual_2016', (2, 2), ['pt', 'rho'] ,basis='Bernstein', limits=(-100, 100))
