...
def autoMCStats_v2(samp, lnN=False, epsilon=0, threshold=0, sample_name=None, bini=None):
    """
    Set MC statical uncertainties based on samp._sumw2. `sample_name` and `bini` parameters
    ...
    sample_name: custom name for e.g. using same parameters in two regions. Uses ``samp.name``
    by default (if sample_name=None).
    bini: create parameter for a specific bin. By default creates for all (if bin=None)
    (only if lnN = False).
    """

    if samp._sumw2 is None:
        raise ValueError("No samp._sumw2 defined in template")
        return

    name = samp._name if sample_name is None else sample_name
    ...
    if lnN:
        ...
        _nom_rate = np.sum(samp._nominal)
        if _nom_rate < 0.0001:
            effect = 1.0
        else:
            print(name+" Nominal Rate: "+str(samp._nominal))
            print(name+" _sumw2: "+str(samp._sumw2))
            _down_rate = np.sum(np.nan_to_num(samp._nominal - np.sqrt(samp._sumw2), 0.0))
            _up_rate = np.sum(np.nan_to_num(samp._nominal + np.sqrt(samp._sumw2), 0.0))
            print(name+" down Rate: "+str(_down_rate))
            print(name+" up Rate: "+str(_up_rate))
            _diff = np.abs(_up_rate - _nom_rate) + np.abs(_down_rate - _nom_rate)
            effect = 1.0 + _diff / (2.0 * _nom_rate)
        param = rl.NuisanceParameter(name + "_mcstat", "lnN")
        samp.setParamEffect(param, effect)
    else:
        if bini is not None:
            assert bini >= 0 and bini < samp.observable.nbins, "autoMCStats bini %d out of range for sample %r " % (bini, samp)

        for i in range(samp.observable.nbins):
            if bini is not None and bini != i:
                continue
            if samp._nominal[i] <= 0.0 or samp._sumw2[i] <= 0.0:
                continue
            effect_up = np.ones_like(samp._nominal)
            effect_down = np.ones_like(samp._nominal)

            if (np.sqrt(samp._sumw2[i]) / (samp._nominal[i] + 1e-12)) < threshold:
                continue

            effect_up[i] = (samp._nominal[i] + np.sqrt(samp._sumw2[i])) / samp._nominal[i]
            effect_down[i] = max((samp._nominal[i] - np.sqrt(samp._sumw2[i])) / samp._nominal[i], epsilon)
            param = rl.NuisanceParameter(name + "_mcstat_bin%i" % i, combinePrior="shape")
            samp.setParamEffect(param, effect_up, effect_down)
...
def get_template(sName, passed, ptbin, obs, syst, muon=False):
    ...
    name = sName+'_'+name+'_'+"jet_pt_soft_wide10"
...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    pts = np.array([0, 220, 250, 280, 320, 380, 700, 2000])
    ...
    ROOT.gInterpreter.Declare("Double_t widebins10[9] = {0, 220, 250, 280, 320, 380, 700, 2000};")
    mpt_TF = TH2F("mpt_TF", "Mass vs. pT Transfer Factor", 7, widebins10, 40, 0, 2000)
...
#	print(mpt[i][2])
    ...
    cb.set_label("Ratio")
    fig.savefig("MCTF_msdpt_2017_sig10v3.png",bbox="tight")
    plt.clf()
    ...
    cb = fig.colorbar(h[3],ax=ax)
    cb.set_label("Ratio")
    fig.savefig("MCTF_rhopt_2017_sig10v3.png",bbox="tight")
...
def test_rhalphabet(tmpdir,
    ...
    # define bins    
    ptbins = np.array([220, 250, 280, 320, 380, 700])
    npt = len(ptbins) - 1
    ...
    rhopts = 2*np.log(msdpts/ptpts)
    print(rhopts)
    ptscaled = (ptpts - 220.0) / (700.0 - 220.0)
    ...
    GJfit_ws.add(GJfit)
    GJfit_ws.writeToFile(os.path.join(str(tmpdir), 'testModel_GJfit_2017.root'))
...
#    plot_mctf(tf_MCtempl,msdbins)
    ...
    decoVector = rl.DecorrelatedNuisanceVector.fromRooFitResult(tf_MCtempl.name + '_deco_2017_', GJfit, param_names)
    tf_MCtempl.parameters = decoVector.correlated_params.reshape(tf_MCtempl.parameters.shape)
    tf_MCtempl_params_final = tf_MCtempl(ptscaled, rhoscaled)
    tf_dataResidual = rl.BasisPoly('CMS_EXO24027_tf_dataResidual_2017', (2, 2), ['pt', 'rho'] ,basis='Bernstein', limits=(-100, 100))
    tf_dataResidual_params = tf_dataResidual(ptscaled, rhoscaled)
    tf_params = GJeff * tf_MCtempl_params_final * tf_dataResidual_params

    # build actual fit model now
    model = rl.Model('testModel_2017')

    ofile_morph = ROOT.TFile("morph_templates.root", "RECREATE")
    ofile_morph.cd()
    
    # exclude GJ from MC samps
    samps = ['TTBar','Sig10', 'WGamma', 'ZGamma']
    sigs = ['Sig10']
    for ptbin in range(npt):
		...
		if do_systematics:
		    ...
		    #autoMCStats_v2(sample, lnN=True)	# Testing to fix sqrt(sumw2) issue
    ...
    for ptbin in range(npt):
        ...
        Dataparams = np.array([rl.IndependentParameter('CMS_EXO24027_Dataparam_ptbin%d_msdbin%d_2017' % (ptbin, i), 0) for i in range(msd.nbins)])
...
#            raise ValueError('initial_Data negative for some bins..', initial_Data)
        ...
        fail_NonRes = rl.ParametericSample('ptbin%dfail_NonRes2017' % ptbin, rl.Sample.BACKGROUND, msd, scaledparams)
        failCh.addSample(fail_NonRes)
        pass_NonRes = rl.TransferFactorSample('ptbin%dpass_NonRes2017' % ptbin, rl.Sample.BACKGROUND, tf_params[ptbin, :], fail_NonRes)
    ...
    with open(os.path.join(str(tmpdir), 'testModel_2017.pkl'), 'wb') as fout:
        pickle.dump(model, fout)

    model.renderCombine(os.path.join(str(tmpdir), 'testModel_2017'))
