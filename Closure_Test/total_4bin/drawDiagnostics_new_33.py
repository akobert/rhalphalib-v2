...
def AddCMSLumi(pad, fb, extra):
	...
	latex.DrawLatex(0.165, 0.84, cmsText)
	pad.Update()
...
def drawDiagnostic(name, ifile1, mass):
	...
	#ptBins = [200, 230, 255, 290, 360, 1000]
    	...
    	ptBins['2016'] = np.array([200, 230, 260, 310, 700])
    	ptBins['2017'] = np.array([220, 250, 285, 340, 700])
    	ptBins['2018'] = np.array([120, 140, 160, 195, 700])
	...
	for y in years:
		for pf in ["pass", "fail"]:
			...
			for a in range(0, 4):
				...
				CbnF = ROOT.TFile("sig25/fitDiagnosticsTest.root") # Get non-signal templates from this one
      ...
      # 	                                 if P == "fit_b":
       #	                                         print("fit_b Test sqrt: "+str(math.sqrt(bkg.GetBinContent(3))))
        	#                                        print("fit_b Test Error: "+str(bkg.GetBinError(3)))
						...
						# Add Chi2 GOF
						chi2, ndf, bins = Chi2(data, bkg, 16) #9 fit parameters for 2,2 Bernstein
						if P == "fit_b" and pf == "pass":
							b_bins += bins
							b_chi += chi2
...
#		year_summary[y]['sig10'].Draw("histsame")
	...
	#	if P != "fit_b": year_summary[y]['sig25'].Draw("histsame")
		...
		# Add Chi2 GOF
		#chi2, ndf, bins = Chi2(year_summary[y]['data'], year_summary[y]['tbkg'], 16) #16 fit parameters for 3,3 Bernstein
		print("Year: "+y)
		print("Chi2: "+str(gof_summary[y]['chi2']))
		print("Bins: "+str(gof_summary[y]['bins']))
		print("NDF: "+str(gof_summary[y]['bins']-16))
		print("Chi2/NDF: "+str(gof_summary[y]['chi2']/(gof_summary[y]['bins']-16)))
		GOF(ROOT.gPad, gof_summary[y]['chi2'], gof_summary[y]['bins']-16)
...
#	if P != "fit_b": b_pass_tsig25_total.Draw("histsame")
	...
	# Add Chi2 GOF
	#chi2, ndf, bins = Chi2(b_pass_data_total, b_pass_tbkg_total, 16) #9 fit parameters for 2,2 Bernstein
	GOF(ROOT.gPad, b_chi, b_bins-48)
...
#	if P != "fit_b": s_pass_tsig25_total.Draw("histsame")
	...
	# Add Chi2 GOF
	#chi2, ndf, bins = Chi2(s_pass_data_total, s_pass_tbkg_total, 16) #9 fit parameters for 2,2 Bernstein
	GOF(ROOT.gPad, s_chi, s_bins-48)
