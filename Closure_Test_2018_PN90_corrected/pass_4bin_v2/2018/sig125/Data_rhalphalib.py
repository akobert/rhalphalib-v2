    ...
    for ptbin in range(npt):
		...
		if do_systematics:
	        ...
	        print("initial data_obs[0]: "+str(data_obs[0]))
                ...
                if ptbin == 0:
                    def func(x): 
	        	return 3.77202*(sin(4.57805*sqrt(x))/x) + 1.03693
                    corr[:5] = np.array([func(x) for x in msdpts[0][:5]])
                    print("Correction: "+str(corr))
                    data_obs[0] = data_obs[0] * corr
                    print("New data_obs: "+str(data_obs[0]))
                if ptbin == 1:
                    def func(x): 
        		return 2.07897*(sin(4.63255*sqrt(x))/x) + 1.01116
                    corr[:5] = np.array([func(x) for x in msdpts[0][:5]])
                    print("Correction: "+str(corr))
                    data_obs[0] = data_obs[0] * corr
                    print("New data_obs: "+str(data_obs[0]))
                if ptbin == 2:
                    def func(x): 
        		return -.301935*(sin(4.19511*sqrt(x))/x) + .980149
                    corr[:5] = np.array([func(x) for x in msdpts[0][:5]])
                    print("Correction: "+str(corr))
                    data_obs[0] = data_obs[0] * corr
                    print("New data_obs: "+str(data_obs[0]))
                if ptbin == 3:
                    def func(x): 
        		return -1.00434*(sin(4.50682*sqrt(x))/x) + .994179
                    corr[:5] = np.array([func(x) for x in msdpts[0][:5]])
                    print("Correction: "+str(corr))
                    data_obs[0] = data_obs[0] * corr
                    print("New data_obs: "+str(data_obs[0]))
