...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    pts = np.array([0, 200, 240, 275, 330, 700, 2000])
    ...
    ROOT.gInterpreter.Declare("Double_t widebins11[9] = {0, 200, 240, 275, 330, 700, 2000};")
...
#	print(mpt[i][2])
    ...
    cb.set_label("Ratio")
    fig.savefig("MCTF_msdpt_2016_sig105v3.png",bbox="tight")
    plt.clf()
    ...
    cb = fig.colorbar(h[3],ax=ax)
    cb.set_label("Ratio")
    fig.savefig("MCTF_rhopt_2016_sig105v3.png",bbox="tight")
...
def test_rhalphabet(tmpdir,
    ...
    """
    with open('/home/akobert/CMSSW_11_3_4/src/FitTest/sf_NEW.json') as f:
        SF = json.load(f)
    ...
    sys_ParticleNeteff = rl.NuisanceParameter('CMS_EXO24027_eff_j_PN_{}'.format(year[:4]), 'lnN')
    ...
    # define bins    
    ptbins = np.array([200, 240, 275, 330, 700])
    npt = len(ptbins) - 1
    ...
    rhopts = 2*np.log(msdpts/ptpts)
    print(rhopts)
    ptscaled = (ptpts - 200.0) / (700.0 - 200.0)
    ...
    initial_vals = initial_vals.reshape(5, 5)
    print(initial_vals)

    tf_MCtempl = rl.BasisPoly('CMS_EXO24027_tf_MCtempl', (4, 4), ['pt', 'rho'], basis='Bernstein', init_params=initial_vals, limits=(-100, 100))
    ...
    GJfit_ws.add(GJfit)
    GJfit_ws.writeToFile(os.path.join(str(tmpdir), 'testModel_GJfit_2016.root'))
...
#    plot_mctf(tf_MCtempl,msdbins)
    ...
    decoVector = rl.DecorrelatedNuisanceVector.fromRooFitResult(tf_MCtempl.name + '_deco_2016_', GJfit, param_names)
    tf_MCtempl.parameters = decoVector.correlated_params.reshape(tf_MCtempl.parameters.shape)
    tf_MCtempl_params_final = tf_MCtempl(ptscaled, rhoscaled)
    tf_dataResidual = rl.BasisPoly('CMS_EXO24027_tf_dataResidual_2016', (2, 3), ['pt', 'rho'] ,basis='Bernstein', limits=(-100, 100))
    tf_dataResidual_params = tf_dataResidual(ptscaled, rhoscaled)
    tf_params = GJeff * tf_MCtempl_params_final * tf_dataResidual_params

    # build actual fit model now
    model = rl.Model('testModel_2016')

    ofile_morph = ROOT.TFile("morph_templates.root", "RECREATE")
    ofile_morph.cd()
    
    # exclude GJ from MC samps
    samps = ['TTBar','Sig105', 'WGamma', 'ZGamma']
    sigs = ['Sig105']
    for ptbin in range(npt):
		...
		if do_systematics:
                    ...
                    sample.setParamEffect(sys_ParticleNeteff,effect)
    ...
    for ptbin in range(npt):
        ...
        Dataparams = np.array([rl.IndependentParameter('CMS_EXO24027_Dataparam_ptbin%d_msdbin%d_2016' % (ptbin, i), 0) for i in range(msd.nbins)])
...
#            raise ValueError('initial_Data negative for some bins..', initial_Data)
        ...
        fail_NonRes = rl.ParametericSample('ptbin%dfail_NonRes2016' % ptbin, rl.Sample.BACKGROUND, msd, scaledparams)
        failCh.addSample(fail_NonRes)
        pass_NonRes = rl.TransferFactorSample('ptbin%dpass_NonRes2016' % ptbin, rl.Sample.BACKGROUND, tf_params[ptbin, :], fail_NonRes)
    ...
    with open(os.path.join(str(tmpdir), 'testModel_2016.pkl'), 'wb') as fout:
        pickle.dump(model, fout)

    model.renderCombine(os.path.join(str(tmpdir), 'testModel_2016'))
