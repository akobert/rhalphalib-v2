...
def FindAndSetMax(*args):
	...
	for j in args:
		...
		j.GetYaxis().SetRangeUser(3,maximum*2.2)
		j.SetLineWidth(2)
	return maximum*1.35
def FindAndSetMax2018(*args):
	if len(args) == 1: args = args[0]
	maximum = 0.0
	for i in args:
		i.SetStats(0)
	...
	for j in args:
		#j.GetYaxis().SetRangeUser(1,maximum*1.35)#should be 1.35 (below as well)
		j.GetYaxis().SetRangeUser(15,maximum*2.2)
		j.SetLineWidth(2)
	return maximum*1.35
...
def AddCMSLumi(pad, fb, extra):
	...
	latex.DrawLatex(0.135, 1-t+lumiTextOffset*t, cmsText)
	pad.Update()
...
#	e = 0.055
#	pad.cd()
#	cmsText     = str(y)+" p_{T} Bin: "+Bin+" GeV"
#	latex = TLatex()
#	latex.SetNDC()
#	latex.SetTextAngle(0)
#	latex.SetTextColor(kBlack)	
...
#	latex.DrawLatex(0.21, 0.81, cmsText)
#	pad.Update()
def pTbin(pad, Bin, y):
	ptTextFont = 42
	ptTextSize     = 0.65
	...
	e = 0.055
	pad.cd()
	ptText = str(y)+" p_{T} Bin: "+Bin+" GeV"
	latex = TLatex()
	latex.SetNDC()
	latex.SetTextAngle(0)
	latex.SetTextColor(kBlack)	
	extraTextSize = 0.76*ptTextSize
	latex.SetTextFont(ptTextFont)
	latex.SetTextAlign(11) 
	latex.SetTextSize(ptTextSize*t)
	latex.DrawLatex(0.21, 0.81,ptText)
	pad.Update()
...
def drawDiagnostic(name, ifile1, mass):
	...
	# Try style macro
	gROOT.LoadMacro("../setTDRStyle_teliko_v2.C")
	setTDRStyle_teliko()	
	...
	CbnF2_10 = ROOT.TFile.Open("sig10/higgsCombineTest.AsymptoticLimits.mH120.root", 'read')
	CbnF2_25 = ROOT.TFile.Open("sig25/higgsCombineTest.AsymptoticLimits.mH120.root", 'read')
	CbnF2_50 = ROOT.TFile.Open("sig50/higgsCombineTest.AsymptoticLimits.mH120.root", 'read')
	CbnF2_75 = ROOT.TFile.Open("sig75/higgsCombineTest.AsymptoticLimits.mH120.root", 'read')
	...
	for y in years:
		for pf in ["pass", "fail"]:
			...
			pull2D[y][pf].GetYaxis().SetTitle("Softdrop Mass [GeV]")
			...
			#for a in range(0, 5):
				...
				CbnF = ROOT.TFile("sig50/fitDiagnosticsTest.root") # Get non-signal templates from this one
				CbnF_10 = ROOT.TFile("sig10/fitDiagnosticsTest.root")
				CbnF_25 = ROOT.TFile("sig25/fitDiagnosticsTest.root")
				CbnF_50 = ROOT.TFile("sig50/fitDiagnosticsTest.root")
				CbnF_75 = ROOT.TFile("sig75/fitDiagnosticsTest.root")
...
#						print(cSIG.GetNbinsX())
						...
						bkg = Hvec[2]
						GoodPlotFormat(bkg,"thickline", ROOT.kBlue, 2)
						#GoodPlotFormat(data,"thickline", ROOT.kBlue, 2)
						ttbar = Hvec[1]
						GoodPlotFormat(ttbar,"fill", ROOT.kRed, ROOT.kRed,  1001, 1)
						sig10 = Hvec[9]
						GoodPlotFormat(sig10, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
						sig25 = Hvec[3]
						GoodPlotFormat(sig25, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
						sig50 = Hvec[4]
						GoodPlotFormat(sig50, "fill", ROOT.kRed+1, ROOT.kGray+1, 3003, 1)
						sig75 = Hvec[5]
						GoodPlotFormat(sig75, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
	                                        WG = Hvec[6]
	                                        GoodPlotFormat(WG,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
	                                        ZG = Hvec[7]
						GoodPlotFormat(ZG,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
						nonRes = Hvec[8]
						GoodPlotFormat(nonRes,"fill", ROOT.kCyan, 10, 1, 2)
	                                        
						tRes = Hvec[6]	# Total Resonant Background
	                                        GoodPlotFormat(tRes,"fill", ROOT.kViolet, ROOT.kViolet, 3335, 1)
	
						cheapline = data.Clone("cheapline")
						cheapline.SetTitle("")
						cheapline.GetXaxis().SetTitle("Softdrop Mass [GeV]")
						cheapline.Add(data,-1.)
						cheapline.GetYaxis().SetTitle("#frac{data - bkg}{#sigma_{data}}")
						cheapline.GetYaxis().SetTitleSize(0.165);
						...
						bkg.GetYaxis().SetTitleOffset(0.6);
						bkg.GetYaxis().SetTitleSize(0.08);
#       	                                 if P == "prefit":
      ...
      # 	                                 if P == "fit_b":
       #	                                         print("fit_b Test sqrt: "+str(math.sqrt(bkg.GetBinContent(3))))
        	#                                        print("fit_b Test Error: "+str(bkg.GetBinError(3)))
						...
						if y != '2018': FindAndSetMax(data, bkg)
						else: FindAndSetMax2018(data, bkg)
...
#						WG.Add(ZG)
#						ttbar.Add(WG)
						# Add ZG and ttbar to total Resonant Background
						tRes.Add(ZG)
						tRes.Add(ttbar)

						#L = ROOT.TLegend(0.65,0.7,0.92,0.88)
						L = ROOT.TLegend(0.695,0.55,0.895,0.88)
...
#	                                        L.AddEntry(WG, "W+Gamma", "F")
#	                                        L.AddEntry(ZG, "Z+Gamma", "F")
						L.AddEntry(nonRes, "Non-Resonant Background", "L")
						L.AddEntry(E[0], "Background Uncertainty", "F")
						L.AddEntry(tRes, "Resonant Backgrounds", "F")
...
#						if run_code == 2: sig75.Draw("histsame")
						...
						l2.Draw("same")
						p22.RedrawAxis()
	#					C.Print("results_pretty/"+NAME+"/"+savename+"_"+pf+str(a)+"_v5.root")
						C.Print("results_pretty/"+NAME+"/"+y+"_"+savename+"_"+pf+str(a)+"_"+str(name)+".png")
						p12.cd()
						gPad.SetLogy()
						p12.Draw()
						C.Print("results_pretty/"+NAME+"/"+y+"_"+savename+"_"+pf+str(a)+"_"+str(name)+"_logy.png")
						if pf == "pass" and P == "fit_b":
...
#								year_summary[y]['sig75'].Add(sig75)
							...
							l2.Draw("same")
							p22.RedrawAxis()
							C.Print("results_pretty/"+NAME+"/"+y+"_multiSig_"+pf+str(a)+"_"+str(name)+".png")
							p12.cd()
							gPad.SetLogy()
							p12.Draw()
							C.Print("results_pretty/"+NAME+"/"+y+"_multiSig_"+pf+str(a)+"_"+str(name)+"_logy.png")
							if pf == "pass":
		...
		GoodPlotFormat(year_summary[y]['tbkg'],"thickline", ROOT.kBlue, 2)
		GoodPlotFormat(year_summary[y]['ttbar'], "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
        	GoodPlotFormat(year_summary[y]['wgamma'],"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
        	GoodPlotFormat(year_summary[y]['zgamma'],"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
        	GoodPlotFormat(year_summary[y]['nres'],"fill", ROOT.kCyan, 10, 1, 2)
		GoodPlotFormat(year_summary[y]['sig10'], "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
		GoodPlotFormat(year_summary[y]['sig25'], "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
		GoodPlotFormat(year_summary[y]['sig50'], "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
		GoodPlotFormat(year_summary[y]['sig75'], "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
		...
		year_cheapline.GetYaxis().SetTitleSize(0.165);
		...
		year_summary[y]['tbkg'].GetYaxis().SetTitleOffset(0.6);
		year_summary[y]['tbkg'].GetYaxis().SetTitleSize(0.08);
		if y != '2018': FindAndSetMax(year_summary[y]['data'], year_summary[y]['tbkg'])
		else: FindAndSetMax2018(year_summary[y]['data'], year_summary[y]['tbkg'])
...
#		year_summary[y]['ttbar'].Add(year_summary[y]['wgamma'])
		...
		L.AddEntry(year_summary[y]['ttbar'], "t#bar{t}", "F")
...
#		year_summary[y]['sig75'].Draw("histsame")
	#	if P != "fit_b": year_summary[y]['sig25'].Draw("histsame")
		...
		l2.Draw("same")
		p22.RedrawAxis()
		C.Print("results_pretty/multiSig_pass_"+y+"_"+str(name)+".png")
		p12.cd()
		gPad.SetLogy()
		p12.Draw()
		C.Print("results_pretty/multiSig_pass_"+y+"_"+str(name)+"_logy.png")
	...
	for y in years:
		for pf in ["pass", "fail"]:
			...
			Cpull.Print("results_pretty/pull2D_"+pf+"_"+str(y)+".png")
	Cpull.Close()	
	...
	GoodPlotFormat(pre_fail_tbkg_total,"thickline", ROOT.kBlue, 2)
	GoodPlotFormat(pre_pass_tbkg_total,"thickline", ROOT.kBlue, 2)
#	GoodPlotFormat(pre_fail_tbkg_total,"fill", ROOT.kBlue, 0, 1001, 1)
#	GoodPlotFormat(pre_pass_tbkg_total,"fill", ROOT.kBlue, 0, 1001, 1)
	GoodPlotFormat(b_fail_tbkg_total,"thickline", ROOT.kBlue, 2)
#	GoodPlotFormat(b_fail_tbkg_total,"fill", ROOT.kBlue, 0, 1001, 1)
#	GoodPlotFormat(b_pass_tbkg_total,"fill", ROOT.kBlue, 0, 1001, 1)
#	GoodPlotFormat(s_fail_tbkg_total,"fill", ROOT.kBlue, 0, 1001, 1)
#	GoodPlotFormat(s_pass_tbkg_total,"fill", ROOT.kBlue, 0, 1001, 1)
	GoodPlotFormat(b_pass_tbkg_total,"thickline", ROOT.kBlue, 2)
	GoodPlotFormat(s_fail_tbkg_total,"thickline", ROOT.kBlue, 2)
	GoodPlotFormat(s_pass_tbkg_total,"thickline", ROOT.kBlue, 2)
#	GoodPlotFormat(pre_fail_ttbar_total,"thickline", ROOT.kRed, 1)
#	GoodPlotFormat(pre_pass_ttbar_total,"thickline", ROOT.kRed, 1)
	GoodPlotFormat(pre_fail_ttbar_total, "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
	GoodPlotFormat(pre_pass_ttbar_total, "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
	GoodPlotFormat(b_fail_ttbar_total, "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
	GoodPlotFormat(b_pass_ttbar_total, "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
	GoodPlotFormat(s_fail_ttbar_total, "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
	GoodPlotFormat(s_pass_ttbar_total, "fill", ROOT.kRed, ROOT.kRed,  1001, 1)
...
#	GoodPlotFormat(s_pass_ttbar_total,"thickline", ROOT.kRed, 1)
	GoodPlotFormat(pre_fail_tsig10_total, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
	GoodPlotFormat(pre_pass_tsig10_total, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
	GoodPlotFormat(b_fail_tsig10_total, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
	GoodPlotFormat(b_pass_tsig10_total, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
	GoodPlotFormat(s_fail_tsig10_total, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
	GoodPlotFormat(s_pass_tsig10_total, "fill", ROOT.kBlue+1, ROOT.kBlue+1, 3003, 1)
	GoodPlotFormat(pre_fail_tsig25_total, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
	GoodPlotFormat(pre_pass_tsig25_total, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
	GoodPlotFormat(b_fail_tsig25_total, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
	GoodPlotFormat(b_pass_tsig25_total, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
	GoodPlotFormat(s_fail_tsig25_total, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
	GoodPlotFormat(s_pass_tsig25_total, "fill", ROOT.kGreen+1, ROOT.kGreen+1, 3003, 1)
	GoodPlotFormat(pre_fail_tsig50_total, "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
	GoodPlotFormat(pre_pass_tsig50_total, "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
	GoodPlotFormat(b_fail_tsig50_total, "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
	GoodPlotFormat(b_pass_tsig50_total, "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
	GoodPlotFormat(s_fail_tsig50_total, "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
	GoodPlotFormat(s_pass_tsig50_total, "fill", ROOT.kGray+1, ROOT.kGray+1, 3003, 1)
	GoodPlotFormat(pre_fail_tsig75_total, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
	GoodPlotFormat(pre_pass_tsig75_total, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
	GoodPlotFormat(b_fail_tsig75_total, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
	GoodPlotFormat(b_pass_tsig75_total, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
	GoodPlotFormat(s_fail_tsig75_total, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
	GoodPlotFormat(s_pass_tsig75_total, "fill", ROOT.kYellow+1, ROOT.kYellow+1, 3003, 1)
...
#        GoodPlotFormat(pre_pass_wgamma_total,"thickline", ROOT.kViolet, 1)
        GoodPlotFormat(pre_fail_wgamma_total,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
        GoodPlotFormat(pre_pass_wgamma_total,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
        GoodPlotFormat(b_fail_wgamma_total,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
        GoodPlotFormat(b_pass_wgamma_total,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
        GoodPlotFormat(s_fail_wgamma_total,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
        GoodPlotFormat(s_pass_wgamma_total,"fill", ROOT.kViolet, ROOT.kViolet, 1001, 1)
...
#        GoodPlotFormat(s_pass_wgamma_total,"thickline", ROOT.kViolet, 1)
      #  GoodPlotFormat(pre_fail_zgamma_total,"thickline", ROOT.kOrange, 1)
       # GoodPlotFormat(pre_pass_zgamma_total,"thickline", ROOT.kOrange, 1)
        GoodPlotFormat(pre_fail_zgamma_total,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
        GoodPlotFormat(pre_pass_zgamma_total,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
        GoodPlotFormat(b_fail_zgamma_total,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
        GoodPlotFormat(b_pass_zgamma_total,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
        GoodPlotFormat(s_fail_zgamma_total,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
        GoodPlotFormat(s_pass_zgamma_total,"fill", ROOT.kOrange, ROOT.kOrange, 1001, 1)
...
#        GoodPlotFormat(pre_pass_nres_total,"dashed", ROOT.kCyan, 1)
        GoodPlotFormat(pre_fail_nres_total,"fill", ROOT.kCyan, 10, 1, 2)
        GoodPlotFormat(pre_pass_nres_total,"fill", ROOT.kCyan, 10, 1, 2)
        GoodPlotFormat(b_fail_nres_total,"fill", ROOT.kCyan, 10, 1, 2)
        GoodPlotFormat(b_pass_nres_total,"fill", ROOT.kCyan, 10, 1, 2)
        GoodPlotFormat(s_fail_nres_total,"fill", ROOT.kCyan, 10, 1, 2)
        GoodPlotFormat(s_pass_nres_total,"fill", ROOT.kCyan, 10, 1, 2)
#        GoodPlotFormat(b_pass_nres_total,"dashed", ROOT.kCyan, 3)
#        GoodPlotFormat(s_fail_nres_total,"dashed", ROOT.kCyan, 3)
#        GoodPlotFormat(s_pass_nres_total,"dashed", ROOT.kCyan, 3)
	...
	b_cheapline.GetYaxis().SetTitleSize(0.165);
	...
	b_pass_tbkg_total.GetYaxis().SetTitleOffset(0.6);
	b_pass_tbkg_total.GetYaxis().SetTitleSize(0.08);
...
#	b_pass_ttbar_total.Add(b_pass_wgamma_total)
	...
	L.AddEntry(b_pass_ttbar_total, "t#bar{t}", "F")
...
#	if P != "fit_b": b_pass_tsig25_total.Draw("histsame")
	...
	l2.Draw("same")
	p22.RedrawAxis()
	C.Print("results_pretty/fit_b_pass_allbins_"+str(name)+".png")
	p12.cd()
	gPad.SetLogy()
	p12.Draw()
	C.Print("results_pretty/fit_b_pass_allbins_"+str(name)+"_logy.png")
	...
	s_cheapline.GetYaxis().SetTitleSize(0.165);
	...
	s_pass_tbkg_total.GetYaxis().SetTitleOffset(0.6);
	s_pass_tbkg_total.GetYaxis().SetTitleSize(0.08);
...
#	s_pass_ttbar_total.Add(s_pass_wgamma_total)
	...
	L.AddEntry(s_pass_ttbar_total, "t#bar{t}", "F")
...
#	if P != "fit_b": s_pass_tsig25_total.Draw("histsame")
	...
	l2.Draw("same")
	p22.RedrawAxis()
	C.Print("results_pretty/multiSig_pass_allbins_"+str(name)+".png")
	p12.cd()
	gPad.SetLogy()
	p12.Draw()
	C.Print("results_pretty/multiSig_pass_allbins_"+str(name)+"_logy.png")
