...
def main():
	#myFunc = TF1("myFunc", "[0]*(sin([1]*sqrt(x))/x)+[2]", 5, 200)
	myFunc = TF1("myFunc", "[0]*x + [1]*x**2 + [2]*x**3 +[3]", 5, 200)
	refF = ROOT.TFile.Open("diagnositcs_33.root", 'read')

	for i in range(0,4):
		myFunc.SetParameters(1,-1,1,1)
#		print(myFunc(12.5))

		hist = refF.Get("2018_data_bkg_ratio_pTbin"+str(i))
	
		hist.Fit(myFunc,"","",5,25)
