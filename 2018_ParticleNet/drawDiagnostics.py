...
def AddCMSLumi(pad, fb, extra):
	...
	latex.DrawLatex(0.215, 0.84, cmsText)
	pad.Update()
...
#					print(cSIG.GetNbinsX())
					...
					if P == "prefit":
						for i in [cDATA, cTT, cTBKG, cSIG25, cSIG50, cSIG75, cWG, cZG, cNRES]:
							i.Scale(5.0)
...
#					C2e.Print("results/"+NAME+"/Sig2D_"+P+".png")
					...
					if P == "fit_b":
						savename25 = "sig25_sdm_"+str(ptBins[a])+"-"+str(ptBins[a+1])
						savename50 = "sig50_sdm_"+str(ptBins[a])+"-"+str(ptBins[a+1])
						savename75 = "sig75_sdm_"+str(ptBins[a])+"-"+str(ptBins[a+1])
					...
					GoodPlotFormat(sig25, "fill", ROOT.kGreen+1, 3003)
					sig50 = Hvec[4]
					GoodPlotFormat(sig50, "fill", ROOT.kGray+1, 3003)
					sig75 = Hvec[5]
					GoodPlotFormat(sig75, "fill", ROOT.kYellow+1, 3003)
                                        WG = Hvec[6]
...
#					C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_v4.root")
					...
					if P == "fit_b":
						...
						l2.Draw("same")
						p22.RedrawAxis()
						C.Print("results/"+NAME+"/multiSig_"+pf+str(a)+"_"+str(name)+".png")
						p12.cd()
						gPad.SetLogy()
						p12.Draw()
						C.Print("results/"+NAME+"/multiSig_"+pf+str(a)+"_"+str(name)+"_logy.png")
						if pf == "pass":
							s_pull_count.SetTitle("multiSig Passing Pull Count "+str(ptBins[a])+"-"+str(ptBins[a+1])+" GeV")
	...
	GoodPlotFormat(pre_fail_tsig25_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(pre_pass_tsig25_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(b_fail_tsig25_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(b_pass_tsig25_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(s_fail_tsig25_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(s_pass_tsig25_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(pre_fail_tsig50_total, "fill", ROOT.kGray+1, 3003)
	GoodPlotFormat(pre_pass_tsig50_total, "fill", ROOT.kGray+1, 3003)
	GoodPlotFormat(b_fail_tsig50_total, "fill", ROOT.kGray+1, 3003)
	GoodPlotFormat(b_pass_tsig50_total, "fill", ROOT.kGray+1, 3003)
	GoodPlotFormat(s_fail_tsig50_total, "fill", ROOT.kGray+1, 3003)
	GoodPlotFormat(s_pass_tsig50_total, "fill", ROOT.kGray+1, 3003)
	GoodPlotFormat(pre_fail_tsig75_total, "fill", ROOT.kYellow+1, 3003)
	GoodPlotFormat(pre_pass_tsig75_total, "fill", ROOT.kYellow+1, 3003)
	GoodPlotFormat(b_fail_tsig75_total, "fill", ROOT.kYellow+1, 3003)
	GoodPlotFormat(b_pass_tsig75_total, "fill", ROOT.kYellow+1, 3003)
	GoodPlotFormat(s_fail_tsig75_total, "fill", ROOT.kYellow+1, 3003)
	GoodPlotFormat(s_pass_tsig75_total, "fill", ROOT.kYellow+1, 3003)
...
#	pTbin(ROOT.gPad, "Full")
	...
	L.AddEntry(s_pass_tsig25_total, "25 GeV Signal", "L")
	L.AddEntry(s_pass_tsig50_total, "50 GeV Signal", "L")
	L.AddEntry(s_pass_tsig75_total, "75 GeV Signal", "L")
	L.AddEntry(E[0], "Background Uncertainty", "F")
#	if P != "fit_b":  L.AddEntry(s_pass_tsig25_total, s_pass_tsig25_total_name, "F")
	...
	p12.cd() # top
	ROOT.gPad.SetTicks(1,1)
	s_pass_tbkg_total.SetTitle("Multiple Signal Background Fit Plot With Signal All Bins")
