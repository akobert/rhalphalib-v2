...
def plot_mctf(tf_MCtempl, msdbins):
    ...
    ROOT.gInterpreter.Declare("Double_t widebins8[10] = {0, 120, 170, 220, 300, 400, 700 ,900, 1200, 1500, 2000};")
    mpt_TF = TH2F("mpt_TF", "Mass vs. pT Transfer Factor", 9, widebins8, 40, 0, 2000)
...
def test_rhalphabet(tmpdir,
    ...
    # Systematics
    sys_lumi = rl.NuisanceParameter('CMS_lumi_13TeV_{}'.format(year[:4]), 'lnN')
    sys_phoID = rl.NuisanceParameter('CMS_phoID_13TeV_{}'.format(year[:4]), 'lnN')
    sys_trig = rl.NuisanceParameter('CMS_trig_13TeV_{}'.format(year[:4]), 'lnN') #Trigger Systematic
    ...
    sys_dict['pu'] = rl.NuisanceParameter('CMS_PU_{}'.format(year), 'lnN')

    sys_ParticleNet_eff = rl.NuisanceParameter('CMS_ParticleNet_eff_{}'.format(year), 'lnN')

    sys_smear = rl.NuisanceParameter('CMS_smear_{}'.format(year), 'shape')
    sys_scale = rl.NuisanceParameter('CMS_scale_{}'.format(year), 'shape')
...
#    plot_mctf(tf_MCtempl,msdbins)
    ...
    tf_dataResidual = rl.BasisPoly('tf_dataResidual', (2, 2), ['pt', 'rho'] ,basis='Bernstein', limits=(-100, 100))
    ...
    for ptbin in range(npt):
		...
		# Lumi Systematic
		sample.setParamEffect(sys_lumi, lumi[year[:4]]['UL'])
		...
		# ParticleNet SF
		sample.scale(SF[year]['ParticleNet_SF'])	# ParticleNet Scale Factor Applied Here
		if do_systematics:
                    effect = 1.0 + SF[year]['ParticleNet_SF_ERR'] / SF[year]['ParticleNet_SF'] # ParticleNet Systematic Applied Here
