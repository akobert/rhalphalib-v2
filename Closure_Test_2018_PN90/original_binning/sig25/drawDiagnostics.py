...
def drawDiagnostic(name, ifile1, mass):
	gROOT.SetBatch(True)
	Sigs = [["/home/akobert/CMSSW_11_1_0_pre7/src/RData/2017/NanoTool_UL/M25_UL_nano_merged.root"], "4.148*7639.0/567896.0", "", "sig25_nano", "25 GeV Signal", "7639.0"]
	sig_name = "sig25_nano"
#	ptBins = [200, 230, 255, 290, 360, 1000]
    	ptBins = {} # Dictonary of bins for each year
    	ptBins['2016'] = np.array([200, 230, 255, 290, 360, 1000])
    	ptBins['2017'] = np.array([220, 230, 255, 290, 360, 1000])
	...
	Blind = True
	LUMI = 13.76 #Temporary luminosity with 10% of Run2 Data
	...
	s_fail_data_total = TH1F("s_fail_data_total", str(mass)+" GeV Failing Data All pT Bins", 40, 0, 200)
	s_pass_data_total = TH1F("s_pass_data_total", str(mass)+" GeV Passing Data All pT Bins", 40, 0, 200)
	...
	pre_fail_tsig_total = TH1F("pre_fail_tsig_total", "Prefit Failing Signal All pT Bins", 40, 0, 200)
	pre_pass_tsig_total = TH1F("pre_pass_tsig_total", "Prefit Passing Signal All pT Bins", 40, 0, 200)
	b_fail_tsig_total = TH1F("b_fail_tsig_total", "Background Only Failing Signal All pT Bins", 40, 0, 200)
	b_pass_tsig_total = TH1F("b_pass_tsig_total", "Background Only Passing Signal All pT Bins", 40, 0, 200)
	s_fail_tsig_total = TH1F("s_fail_tsig_total", str(mass)+" GeV Failing Signal All pT Bins", 40, 0, 200)
	s_pass_tsig_total = TH1F("s_pass_tsig_total", str(mass)+" GeV Passing Signal All pT Bins", 40, 0, 200)
	...
	CbnF2 = ROOT.TFile.Open("higgsCombineTest.AsymptoticLimits.mH120.root", 'read')
	tree = CbnF2.Get("limit")
	limits = []
	for quantile in tree:
		limits.append(tree.limit)
	CL95 = limits[2]  # 95% confidence r value
	print("95% CL r value: "+str(CL95))
	...
	for y in years:
		for pf in ["pass", "fail"]:
			for a in range(0, 5):
				...
				refH = refF.Get("Data_"+pf+"_soft_"+y)
...
#				for P in ["prefit", "fit_b"]:
						...
						cDATA = CbnF.Get("shapes_"+P+"/"+NAME+y+"/data")
						cNRES = CbnF.Get("shapes_"+P+"/"+NAME+y+"/NonRes")	#Non-Resonant Background
						cTT = CbnF.Get("shapes_"+P+"/"+NAME+y+"/TTBar")
					#	cTT = cTT.Clone()
						cTBKG = CbnF.Get("shapes_"+P+"/"+NAME+y+"/total_background")
						if run_code != 2:
							cSIG = CbnF.Get("shapes_"+P+"/"+NAME+y+"/total_signal")
						else:
							cSIG = CbnF.Get("shapes_prefit/"+NAME+y+"/total_signal")	# Take pre-fit signal to scale
	                                        cWG = CbnF.Get("shapes_"+P+"/"+NAME+y+"/WGamma")
	                                        cZG = CbnF.Get("shapes_"+P+"/"+NAME+y+"/ZGamma")
...
#						print(cSIG.GetNbinsX())
						...
						if P == "prefit" and pf == "fail":
							...
							s_fail_tsig_total.Add(Hvec[3])
							s_fail_wgamma_total.Add(Hvec[4])
							s_fail_zgamma_total.Add(Hvec[5])
							s_fail_nres_total.Add(Hvec[6])
						
						elif P == "fit_b" and pf == "pass":
							b_pass_data_total.Add(Hvec[0])
							b_pass_ttbar_total.Add(Hvec[1])
							b_pass_tbkg_total.Add(Hvec[2].Clone())
							print("b_pass_tbkg_total bin 3="+str(b_pass_tbkg_total.GetBinContent(3)))
							print("b_pass_tbkg_total bin 3 err="+str(b_pass_tbkg_total.GetBinError(3)))
							...
							s_pass_tbkg_total.Add(Hvec[2].Clone())
							print("s_pass_tbkg_total bin 3="+str(s_pass_tbkg_total.GetBinContent(3)))
							print("s_pass_tbkg_total bin 3 err="+str(s_pass_tbkg_total.GetBinError(3)))
							s_pass_tsig_total.Add(Hvec[3])
							s_pass_wgamma_total.Add(Hvec[4])
							s_pass_zgamma_total.Add(Hvec[5])
							s_pass_nres_total.Add(Hvec[6])
	...
	#					C2e.Print("results/"+NAME+"/Sig2D_"+P+".png")
						...
						if P == "fit_b":
							savename2 = "sig_sdm_"+str(ptBins[y][a])+"-"+str(ptBins[y][a+1])
...
#							print("bin content="+str(bkg.GetBinContent(i)))
#							print("bin sqrt(content)="+str(sqrt(bkg.GetBinContent(i))))
#							print("bin error="+str(bkg.GetBinError(i)))
...
#									print("fit_b pTbin #"+str(a)+" bin #"+str(i+1)+" Data content: "+str(data.GetBinContent(i+1)))
#									print("fit_b pTbin #"+str(a)+" bin #"+str(i+1)+" Background content: "+str(bkg.GetBinContent(i+1)))
#									print("fit_b pTbin #"+str(a)+" bin #"+str(i+1)+" Data Error: "+str(data.GetBinError(i+1)))
#									print("fit_b pTbin #"+str(a)+" bin #"+str(i+1)+" Pull content: "+str(pull.GetBinContent(i+1)))
								if pf == "pass" and run_code == 3:
									s_pull_count.Fill(pull.GetBinContent(i+1))
									s_pull_count_total.Fill(pull.GetBinContent(i+1)) # Same Pull Count For fit_b and sig (double check this)
#									print("sig pTbin #"+str(a)+" bin #"+str(i+1)+" Signal value: "+str(sig.GetBinContent(i+1)))
#									print("sig pTbin #"+str(a)+" bin #"+str(i+1)+" Data content: "+str(data.GetBinContent(i+1)))
#									print("sig pTbin #"+str(a)+" bin #"+str(i+1)+" Background content: "+str(bkg.GetBinContent(i+1)))
#									print("sig pTbin #"+str(a)+" bin #"+str(i+1)+" Data Error: "+str(data.GetBinError(i+1)))
#									print("sig pTbin #"+str(a)+" bin #"+str(i+1)+" Pull content: "+str(pull.GetBinContent(i+1)))
							else:
	...
	#					C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_v5.root")
						C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_"+y+"_"+str(name)+".png")
						p12.cd()
						gPad.SetLogy()
						p12.Draw()
						C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_"+y+"_"+str(name)+"_logy.png")
						if pf == "pass" and P == "fit_b":
						...
						if P == "fit_b":
							...
							l2.Draw("same")
							p22.RedrawAxis()
							C.Print("results/"+NAME+"/"+savename2+"_"+pf+str(a)+"_"+y+"_"+str(name)+".png")
							p12.cd()
							gPad.SetLogy()
							p12.Draw()
							C.Print("results/"+NAME+"/"+savename2+"_"+pf+str(a)+"_"+y+"_"+str(name)+"_logy.png")
							if pf == "pass":
								s_pull_count.SetTitle("sig Passing Pull Count "+str(ptBins[y][a])+"-"+str(ptBins[y][a+1])+" GeV")
								pull_name = "sig_"+pf+"_pull_count_bin"+str(a)
	...
	GoodPlotFormat(s_fail_tsig_total, "fill", ROOT.kGreen+1, 3003)
	GoodPlotFormat(s_pass_tsig_total, "fill", ROOT.kGreen+1, 3003)
	...
	ofile.WriteObject(s_pull_count_total, "sig_pass_pull_count_total")
	b_pull_count.Reset()
	s_pull_count.Reset()
	...
	ofile2.WriteObject(s_fail_data_total, "sig_fail_data_total")
	ofile2.WriteObject(s_pass_data_total, "sig_pass_data_total")
	...
	ofile2.WriteObject(s_fail_ttbar_total, "sig_fail_ttbar_total")
	ofile2.WriteObject(s_pass_ttbar_total, "sig_pass_ttbar_total")
	...
	ofile2.WriteObject(s_fail_tbkg_total, "sig_fail_tbkg_total")
	ofile2.WriteObject(s_pass_tbkg_total, "sig_pass_tbkg_total")
	...
	ofile2.WriteObject(s_fail_tsig_total, "sig_fail_tsig_total")
	ofile2.WriteObject(s_pass_tsig_total, "sig_pass_tsig_total")
	...
	ofile2.WriteObject(s_fail_wgamma_total, "sig_fail_wgamma_total")
	ofile2.WriteObject(s_pass_wgamma_total, "sig_pass_wgamma_total")
	...
	ofile2.WriteObject(s_fail_zgamma_total, "sig_fail_zgamma_total")
	ofile2.WriteObject(s_pass_zgamma_total, "sig_pass_zgamma_total")
	...
	ofile2.WriteObject(s_fail_nres_total, "sig_fail_nres_total")
	ofile2.WriteObject(s_pass_nres_total, "sig_pass_nres_total")
	...
	for i in range(1,b_pass_data_total.GetNbinsX()+1):
		...
		print("Content = "+str(b_pass_tbkg_total.GetBinContent(i)))
		print("Err = "+str(Err))
...
#	if P != "fit_b":  L.AddEntry(b_pass_tsig_total, b_pass_tsig_total_name, "F")
	...
	p12.cd() # top
	ROOT.gPad.SetTicks(1,1)
	b_pass_tbkg_total.SetTitle("Background Only Fit Plot All Bins")
...
#	pTbin(ROOT.gPad, "Full")
	...
	L.AddEntry(s_pass_ttbar_total, "t#bar{t} component", "L")
        L.AddEntry(s_pass_wgamma_total, "W+Gamma", "L")
        L.AddEntry(s_pass_zgamma_total, "Z+Gamma", "L")
	L.AddEntry(s_pass_nres_total, "Non-Resonant Background", "L")
	L.AddEntry(s_pass_tsig_total, str(mass)+" GeV Signal", "L")
	L.AddEntry(E[0], "Background Uncertainty", "F")
#	if P != "fit_b":  L.AddEntry(s_pass_tsig_total, s_pass_tsig_total_name, "F")
	C = ROOT.TCanvas()
	C.cd()
	p12 = ROOT.TPad("pad1", "tall",0,0.165,1,1)
	p22 = ROOT.TPad("pad2", "short",0,0.0,1.0,0.23)
	...
	p12.cd() # top
	ROOT.gPad.SetTicks(1,1)
	s_pass_tbkg_total.SetTitle(str(mass)+" GeV Background Fit Plot With Signal All Bins")
	...
	s_pass_tsig_total.Draw("histsame")
#	if P != "fit_b": s_pass_tsig_total.Draw("histsame")
	for e in E: e.Draw("same")
...
#	pTbin(ROOT.gPad, "Full")
	...
	l2.Draw("same")
	p22.RedrawAxis()
	C.Print("results/sig_pass_allbins_"+str(name)+".png")
	p12.cd()
	gPad.SetLogy()
	p12.Draw()
	C.Print("results/sig_pass_allbins_"+str(name)+"_logy.png")
