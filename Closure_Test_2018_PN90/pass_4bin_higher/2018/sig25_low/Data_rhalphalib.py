...
def plot_mctf(tf_MCtempl, msdbins, GJeff):
    ...
    rhopts_scaled = (rhopts - (-7.3)) / ((-1.7) - (-7.3))
    ...
    ROOT.gInterpreter.Declare("Double_t widebins15[6] = {120, 160, 200, 255, 700};")
    mpt_TF = TH2F("mpt_TF", "Mass vs. pT Transfer Factor", 5, widebins15, 40, 0, 2000)
...
#	print(mpt[i][2])
    ...
    cb.set_label("Ratio")
    fig.savefig("MCTF_msdpt_2018_sig25v5.png",bbox="tight")
    plt.clf()

    # arrays for plotting pt vs rho                                          
    rhos = np.linspace(-7.3,-1.7,41)
    ...
    cb = fig.colorbar(h[3],ax=ax)
    cb.set_label("Ratio")
    fig.savefig("MCTF_rhopt_2018_sig25v5.png",bbox="tight")
...
def test_rhalphabet(tmpdir,
    ...
    initial_vals = initial_vals.reshape(3, 4)
    print(initial_vals)

    tf_MCtempl = rl.BasisPoly('CMS_EXO24027_tf_MCtempl', (2, 3), ['pt', 'rho'], basis='Bernstein', init_params=initial_vals, limits=(-100, 100))
    ...
    tf_dataResidual = rl.BasisPoly('CMS_EXO24027_tf_dataResidual_2018', (2, 3), ['pt', 'rho'] ,basis='Bernstein', limits=(-100, 100))
