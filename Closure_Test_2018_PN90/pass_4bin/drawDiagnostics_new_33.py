...
def drawDiagnostic(name, ifile1, mass):
	...
	ofile = ROOT.TFile("./fit_b_pull_count_33.root", "RECREATE")
	ofile2 = ROOT.TFile("./diagnositcs_33.root", "RECREATE")
	...
	# Data / TBkg Ratio in each pT bin (2018 only)
	ratios = {}
	for a in range(0, 4):
		ratios[a] = TH1F("pass_data_bkg_ratio_pTbin"+str(a), "Passing (Data / Total Background) Ratio pT Bin #"+str(a), 40, 0, 200)
	...
	#					C.Print("results/"+NAME+"/"+savename+"_"+pf+str(a)+"_v5.root")
						...
						if P == "fit_b":
							...
							if pf == "pass":
								...
								# Fill Ratio Plot
								if y == '2018':
									ratios[a] = data.Clone("2018_ratio_pTbin"+str(a))
									ratios[a].Divide(bkg)
									#ratios[a] = ratios[a].Clone()
									GoodPlotFormat(ratios[a], "markers", ROOT.kBlack, 20)	
									ratios[a].GetYaxis().SetRangeUser(0.2, 1.8)
									C = ROOT.TCanvas()
									C.cd()
									ratios[a].GetXaxis().SetTitle("Softdrop Mass")
									ratios[a].GetYaxis().SetTitle("Data/Background Ratio")
									ratios[a].Draw("histe")
									ofile2.WriteObject(ratios[a], "2018_data_bkg_ratio_pTbin"+str(a))
									C.Print("results/2018_low_data_bkg_ratio_pTbin"+str(a)+".png")
									C.Close()
